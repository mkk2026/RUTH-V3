        ...
        >
            ...
            >
                {messages.slice(-5).map((msg, i) => (
                    <div key={i} className="text-sm border-l-2 border-amber-800/50 pl-3 py-1">
                        <span className="text-amber-600 font-mono text-xs opacity-70">[{msg.time}]</span> <span className="font-bold text-amber-300 drop-shadow-sm">{msg.sender}</span>
            ...
            <div className="flex gap-2 relative z-10 absolute bottom-4 left-6 right-6">
                <input
                    ...
                    className="flex-1 bg-black/40 border border-amber-700/30 rounded-lg p-3 text-amber-50 focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400/50 transition-all placeholder-amber-800/50 backdrop-blur-sm"
                />
            </div>
