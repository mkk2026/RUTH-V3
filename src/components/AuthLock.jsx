...
const AuthLock = ({ socket, onAuthenticated, onAnimationComplete }) => {
    ...
    const themeColor = isUnlocking ? 'text-green-500' : 'text-amber-500';
    const borderColor = isUnlocking ? 'border-green-500' : 'border-amber-500';
    const shadowColor = isUnlocking ? 'shadow-[0_0_50px_rgba(34,197,94,0.4)]' : 'shadow-[0_0_50px_rgba(251,191,36,0.2)]';
            ...
            <div className={`relative flex flex-col items-center gap-6 p-10 border ${borderColor}/30 rounded-lg bg-black/80 backdrop-blur-xl ${shadowColor} transition-all duration-[1500ms]`}>
                ...
                <div className={`relative w-64 h-64 border-2 ${borderColor}/50 rounded-lg overflow-hidden bg-gray-900 shadow-inner flex items-center justify-center transition-colors duration-500`}>
                    {frameSrc ? (
                        ...
                        <div className={`animate-pulse ${isUnlocking ? 'text-green-800' : 'text-amber-800'}`}>
                            <User size={64} />
                        </div>
                    )}
                    ...
                    {!isUnlocking && (
                        <div className="absolute top-0 left-0 w-full h-1 bg-amber-400/80 shadow-[0_0_15px_#fbbf24] animate-[scan_2s_ease-in-out_infinite]"></div>
                    )}
                ...
                <div className={`text-sm tracking-widest ${isUnlocking ? 'text-green-300' : 'text-amber-300'} animate-pulse transition-colors duration-500`}>
                    {message}
                </div>
            </div>
