...
const LoadingCube = () => {
    ...
    return (
        <mesh ref={meshRef}>
            ...
            <meshStandardMaterial wireframe color="#fbbf24" transparent opacity={0.5} />
        </mesh>
    );
};
...
const CadWindow = ({ data, thoughts, retryInfo = {}, onClose, socket }) => {
    ...
    return (
        <div className="w-full h-full relative group bg-gray-900 rounded-lg overflow-hidden border border-amber-500/30">
            {/* Close Button */}
            ...
            <div className="absolute top-2 left-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity flex gap-2">
                <button
                    ...
                    className="bg-amber-500/20 hover:bg-amber-500/50 text-amber-400 text-xs px-2 py-1 rounded border border-amber-500/30 backdrop-blur-sm"
                >
                    ITERATE
                </button>
                <button
            ...
            {(isIterating || (!data && data?.format !== 'loading')) && (
                <div className={`absolute inset-0 z-20 ${!data ? 'bg-gray-900' : 'bg-black/80'} flex items-center justify-center p-4`}>
                    <div className="bg-gray-800 border border-amber-500/50 rounded p-4 w-full max-w-sm pointer-events-auto shadow-[0_0_20px_rgba(245,158,11,0.2)]">
                        <h4 className="text-amber-400 text-sm mb-2 font-mono">
                        ...
                        <textarea
                            ...
                            className="w-full bg-gray-900 border border-gray-700 rounded p-2 text-white text-sm mb-3 focus:outline-none focus:border-amber-500 h-24 resize-none"
                            autoFocus
                            onKeyDown={(e) => {
            ...
            {data?.format === 'loading' && (
                <div className="absolute inset-y-0 right-0 w-2/5 p-4 bg-black/70 backdrop-blur-sm border-l border-green-500/30 overflow-hidden flex flex-col">
                    <div className="flex items-center justify-between mb-2">
                        ...
                        {retryInfo.attempt && (
                            <span className={`text-xs font-mono px-2 py-0.5 rounded ${retryInfo.error ? 'bg-yellow-500/20 text-yellow-400' : 'bg-amber-500/20 text-amber-400'}`}>
            ...
            <div className="absolute bottom-2 left-2 text-[10px] text-amber-500/50 font-mono tracking-widest pointer-events-none">
