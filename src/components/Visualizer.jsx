...
const Visualizer = ({ audioDataRef, isListening, width = 600, height = 400 }) => {
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            // Calculate current intensity from the ref data
            let currentIntensity = 0;
            if (audioDataRef && audioDataRef.current && audioDataRef.current.length > 0) {
                const aiAudioData = audioDataRef.current;
                currentIntensity = aiAudioData.reduce((a, b) => a + b, 0) / aiAudioData.length / 255;
            }
