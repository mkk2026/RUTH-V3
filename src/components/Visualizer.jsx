...
const Visualizer = ({ audioDataRef, isListening, width = 600, height = 400 }) => {
    ...
    // Keep a ref for isListening to avoid re-creating the animation loop on every frame
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            // Calculate intensity locally instead of passing it as a prop
            let currentIntensity = 0;
            if (audioDataRef && audioDataRef.current && audioDataRef.current.length > 0) {
                currentIntensity = audioDataRef.current.reduce((a, b) => a + b, 0) / audioDataRef.current.length / 255;
            }
