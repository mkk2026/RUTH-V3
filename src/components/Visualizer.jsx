...
const Visualizer = ({ socket, isListening, width = 600, height = 400 }) => {
    ...
    // Use a ref to avoid re-creating the animation loop on every frame
    ...
    useEffect(() => {
        ...
        const handleAudioData = (data) => {
            ...
            if (aiAudioData && aiAudioData.length > 0) {
                audioDataRef.current = aiAudioData;
                const amp = aiAudioData.reduce((a, b) => a + b, 0) / aiAudioData.length / 255;
                intensityRef.current = amp;
            }
        };
