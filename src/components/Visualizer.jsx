...
const Visualizer = ({ audioDataRef, isListening, width = 600, height = 400 }) => {
    ...
    useEffect(() => {
        ...
        const draw = () => {
            ...
            // Use current audio data from ref to calculate intensity on the fly
            const audioData = audioDataRef.current || [];
            const calculatedIntensity = audioData.length > 0
                ? audioData.reduce((a, b) => a + b, 0) / audioData.length / 255
                : 0;
            const currentIsListening = isListeningRef.current;

            const baseRadius = Math.min(w, h) * 0.25;
            const radius = baseRadius + (calculatedIntensity * 40);
