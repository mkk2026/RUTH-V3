...
const SettingsWindow = ({
    ...
    return (
        <div className="absolute top-20 right-10 bg-black/90 border border-amber-500/50 p-4 rounded-lg z-50 w-80 backdrop-blur-xl shadow-[0_0_30px_rgba(245,158,11,0.2)]">
            <div className="flex justify-between items-center mb-4 border-b border-amber-900/50 pb-2">
                <h2 className="text-amber-400 font-bold text-sm uppercase tracking-wider">Settings</h2>
            ...
            <div className="mb-6">
                <h3 className="text-amber-400 font-bold mb-3 text-xs uppercase tracking-wider opacity-80">Security</h3>
                <div className="flex items-center justify-between text-xs bg-gray-900/50 p-2 rounded border border-amber-900/30">
                    <span className="text-amber-100/80">Face Authentication</span>
                    <button
                        onClick={toggleFaceAuth}
                        className={`relative w-8 h-4 rounded-full transition-colors duration-200 ${faceAuthEnabled ? 'bg-amber-500/80' : 'bg-gray-700'}`}
                    >
                        <div
            ...
            {/* Microphone Section */}
            <div className="mb-4">
                <h3 className="text-amber-400 font-bold mb-2 text-xs uppercase tracking-wider opacity-80">Microphone</h3>
                <select
                    value={selectedMicId}
                    onChange={(e) => setSelectedMicId(e.target.value)}
                    className="w-full bg-gray-900 border border-amber-800 rounded p-2 text-xs text-amber-100 focus:border-amber-400 outline-none"
                >
            ...
            {/* Speaker Section */}
            <div className="mb-4">
                <h3 className="text-amber-400 font-bold mb-2 text-xs uppercase tracking-wider opacity-80">Speaker</h3>
                <select
            ...
            {/* Webcam Section */}
            <div className="mb-6">
                <h3 className="text-amber-400 font-bold mb-2 text-xs uppercase tracking-wider opacity-80">Webcam</h3>
                <select
            ...
            <div className="mb-6">
                <div className="flex justify-between mb-2">
                    <h3 className="text-amber-400 font-bold text-xs uppercase tracking-wider opacity-80">Cursor Sensitivity</h3>
                    <span className="text-xs text-amber-500">{cursorSensitivity}x</span>
                </div>
                <input
                    type="range"
                    min="1.0"
            ...
            <div className="mb-6">
                <h3 className="text-amber-400 font-bold mb-3 text-xs uppercase tracking-wider opacity-80">Gesture Control</h3>
                <div className="flex items-center justify-between text-xs bg-gray-900/50 p-2 rounded border border-amber-900/30">
                    <span className="text-amber-100/80">Flip Camera Horizontal</span>
                    <button
                        onClick={toggleCameraFlip}
                        className={`relative w-8 h-4 rounded-full transition-colors duration-200 ${isCameraFlipped ? 'bg-amber-500/80' : 'bg-gray-700'}`}
                    >
                        <div
            ...
            <div className="mb-6">
                <h3 className="text-amber-400 font-bold mb-3 text-xs uppercase tracking-wider opacity-80">Tool Confirmations</h3>
                <div className="space-y-2 max-h-40 overflow-y-auto pr-2 custom-scrollbar">
                    {TOOLS.map(tool => {
                        const isRequired = permissions[tool.id] !== false; // Default True
                        return (
                            <div key={tool.id} className="flex items-center justify-between text-xs bg-gray-900/50 p-2 rounded border border-amber-900/30">
                                <span className="text-amber-100/80">{tool.label}</span>
                                <button
                                    onClick={() => togglePermission(tool.id)}
                                    className={`relative w-8 h-4 rounded-full transition-colors duration-200 ${isRequired ? 'bg-amber-500/80' : 'bg-gray-700'}`}
                                >
                                    <div
            ...
            {/* Memory Section */}
            <div>
                <h3 className="text-amber-400 font-bold mb-2 text-xs uppercase tracking-wider opacity-80">Memory Data</h3>
                <div className="flex flex-col gap-2">
                    <label className="text-[10px] text-amber-500/60 uppercase">Upload Memory Text</label>
                    <input
                        type="file"
                        accept=".txt"
                        onChange={handleFileUpload}
                        className="text-xs text-amber-100 bg-gray-900 border border-amber-800 rounded p-2 file:mr-2 file:py-1 file:px-2 file:rounded-full file:border-0 file:text-[10px] file:font-semibold file:bg-amber-900 file:text-amber-400 hover:file:bg-amber-800 cursor-pointer"
                    />
                </div>
            </div>
        </div>
