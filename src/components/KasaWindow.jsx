...
const KasaWindow = ({
    ...
    return (
        <div
            ...
            className={`absolute flex flex-col gap-2 p-4 rounded-xl backdrop-blur-md bg-black/60 border border-amber-500/30 transition-all duration-200 select-none
                ${activeDragElement === 'kasa' ? 'ring-2 ring-green-500 shadow-[0_0_30px_rgba(34,197,94,0.3)]' : 'shadow-[0_0_20px_rgba(245,158,11,0.1)]'}
            `}
            style={{
                left: position.x,
                top: position.y,
        ...
        >
            ...
            <div data-drag-handle className="flex items-center justify-between pb-2 border-b border-white/10 mb-2 cursor-grab active:cursor-grabbing">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-amber-400 tracking-wider text-sm">SMART CONTROL</h3>
                </div>
                <button
                    onClick={onClose}
            ...
            <div className="flex-1 overflow-y-auto max-h-[400px] scrollbar-hide">
                ...
                {devices.length === 0 && !isThinking && (
                    <div className="flex flex-col items-center justify-center p-8 text-center opacity-50">
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-amber-900/30 border border-amber-500/30 rounded-lg hover:bg-amber-500/20 hover:border-amber-500 transition-all text-xs font-mono text-amber-300"
                        >
                ...
                {isThinking && (
                    <div className="flex flex-col items-center justify-center p-8 gap-3">
                        ...
                        <span className="text-xs text-amber-400 animate-pulse">Scanning Network...</span>
                    </div>
                )}

                {devices.map((dev) => (
                    <div key={dev.ip} className="mb-3 p-3 bg-white/5 rounded-lg border border-white/10 hover:border-amber-500/30 transition-all">
                        ...
                        {dev.has_brightness && dev.is_on && (
                            <div className="flex items-center gap-2 mt-2">
                                ...
                                <input
                                    ...
                                    className="w-full h-1 bg-white/10 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-amber-400"
                                />
                            </div>
                        )}
