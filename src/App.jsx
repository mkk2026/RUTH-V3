...
function App() {
    ...
    const [elementPositions, setElementPositions] = useState({
        ...
        terminal: { x: window.innerWidth / 2 - 250, y: window.innerHeight / 2 + 50 },
        scheduler: { x: window.innerWidth / 2 + 250, y: window.innerHeight / 2 + 50 },
        alerts: { x: window.innerWidth / 2 + 350, y: window.innerHeight / 2 },
        integrations: { x: window.innerWidth / 2 - 350, y: window.innerHeight / 2 + 50 },
        messaging: { x: window.innerWidth / 2 + 300, y: window.innerHeight / 2 - 50 },
        plugins: { x: window.innerWidth / 2 - 300, y: window.innerHeight / 2 - 50 },
        modelSettings: { x: window.innerWidth / 2, y: window.innerHeight / 2 - 50 },
        memoryViewer: { x: window.innerWidth / 2, y: window.innerHeight / 2 + 100 }
    });
    ...
    const [elementSizes, setElementSizes] = useState({
        ...
        scheduler: { w: 420, h: 500 },
        alerts: { w: 420, h: 500 },
        integrations: { w: 380, h: 420 },
        messaging: { w: 420, h: 520 },
        plugins: { w: 380, h: 420 },
        modelSettings: { w: 420, h: 480 },
        memoryViewer: { w: 420, h: 480 }
    });
    ...
    const [zIndexOrder, setZIndexOrder] = useState([
        ...
        'terminal', 'scheduler', 'alerts', 'integrations', 'messaging', 'plugins', 'modelSettings', 'memoryViewer'
    ]);
    ...
    // Auto-Connect Model on Start (Only after Auth and socket ready)
    useEffect(() => {
        if (!isConnected || !isAuthenticated || !socketConnected || hasAutoConnectedRef.current) return;
        ...
        const timer = setTimeout(() => {
            const index = micDevices.length > 0
                ? micDevices.findIndex(d => d.deviceId === selectedMicId)
                : -1;
    ...
    useEffect(() => {
        ...
        socket.on('status', (data) => {
            ...
            if (data.msg === 'R.U.T.H. Started') {
                setStatus('Model Connected');
            } else if (data.msg === 'R.U.T.H. Stopped') {
                setStatus('Connected');
            }
        });
        socket.on('audio_data', (data) => {
            aiAudioDataRef.current = data.data;
            audioAmpRef.current = data.data.reduce((a, b) => a + b, 0) / data.data.length / 255;
        });
            ...
            if (results.landmarks && results.landmarks.length > 0) {
                ...
                if (snapStateRef.current.isSnapped) {
                    ...
                    if (closest && minDist < SNAP_THRESHOLD) {
                        ...
                        closest.el.style.boxShadow = '0 0 20px rgba(251, 191, 36, 0.6)';
                        closest.el.style.backgroundColor = 'rgba(245, 158, 11, 0.2)';
                        closest.el.style.borderColor = 'rgba(251, 191, 36, 1)';
                    }
                }
    ...
    // Audio amplitude is now calculated via socket event and stored in audioAmpRef
    ...
    return (
        <div className="h-screen w-screen bg-black text-amber-100 font-mono overflow-hidden flex flex-col relative selection:bg-amber-900 selection:text-white">
            ...
            <div
                className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-amber-900/10 rounded-full blur-[120px] pointer-events-none"
            />

            {/* Top Bar (Draggable) */}
            <div className="z-50 flex items-center justify-between p-2 border-b border-amber-500/20 bg-black/40 backdrop-blur-md select-none sticky top-0" style={{ WebkitAppRegion: 'drag' }}>
                <div className="flex items-center gap-4 pl-2">
                    <h1 className="text-xl font-bold tracking-[0.2em] text-amber-400 drop-shadow-[0_0_10px_rgba(251,191,36,0.5)]">
                        R.U.T.H.
                    </h1>
                    <div className="text-[10px] text-amber-700 border border-amber-900 px-1 rounded">
                        V3.0.0
                    </div>
                    {/* FPS Counter */}
                    {isVideoOn && (
                ...
                <div className="flex-1 flex justify-center mx-4">
                    <TopAudioBar getAudioData={() => micAudioDataRef.current} />
                </div>

                <div className="flex items-center gap-2 pr-2" style={{ WebkitAppRegion: 'no-drag' }}>
                    {/* Live Clock */}
                    <div className="flex items-center gap-1.5 text-[11px] text-amber-300/70 font-mono px-2">
                        <Clock size={12} className="text-amber-500/50" />
                        <span>{currentTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                    <button onClick={handleMinimize} className="p-1 hover:bg-amber-900/50 rounded text-amber-500 transition-colors">
                        <Minus size={18} />
                    </button>
                    <button onClick={handleMaximize} className="p-1 hover:bg-amber-900/50 rounded text-amber-500 transition-colors">
            ...
            <div className="flex-1 relative z-10 flex flex-col items-center justify-center">
                ...
                >
                    ...
                    <div className="relative z-20">
                        <Visualizer
                            getAudioData={() => aiAudioDataRef.current}
                            isListening={isConnected && !isMuted}
                            getIntensity={() => audioAmpRef.current}
                            width={elementSizes.visualizer.w}
                            height={elementSizes.visualizer.h}
                        />
                    </div>
                ...
                <div className="absolute top-[70px] left-1/2 -translate-x-1/2 text-amber-500 text-xs font-mono tracking-widest pointer-events-none z-50 bg-black/50 px-2 py-1 rounded backdrop-blur-sm border border-amber-500/20">
                ...
                >
                    ...
                    <div className="relative border border-amber-500/30 rounded-lg overflow-hidden shadow-[0_0_20px_rgba(245,158,11,0.1)] w-80 aspect-video bg-black/80">
                        ...
                        <div className="absolute top-2 left-2 text-[10px] text-amber-400 bg-black/60 backdrop-blur px-2 py-0.5 rounded border border-amber-500/20 z-10 font-bold tracking-wider">CAM_01</div>
                ...
                {showCadWindow && (
                    ...
                    >
                        ...
                        <div
                            data-drag-handle
                            className="h-8 bg-gray-900/80 border-b border-amber-500/20 flex items-center justify-between px-3 cursor-grab active:cursor-grabbing shrink-0"
                        >
                            <span className="text-xs font-bold tracking-widest text-amber-500/70">CAD PROTOTYPE</span>
                            <button
                ...
                <div className="z-20 flex justify-center pb-10 pointer-events-none">
                    <ToolsModule
                        ...
                        onTogglePlugins={() => setShowPlugins(!showPlugins)}
                        showPlugins={showPlugins}
                        onToggleModelSettings={() => setShowModelSettings(!showModelSettings)}
                        showModelSettings={showModelSettings}
                        onToggleMemoryViewer={() => setShowMemoryViewer(!showMemoryViewer)}
                ...
                {showTerminal && (
                    <TerminalWindow
                        ...
                        position={elementPositions.terminal}
                ...
                {showScheduler && (
                    <SchedulerWindow
                        ...
                        position={elementPositions.scheduler}
                        onMouseDown={(e) => handleMouseDown(e, 'scheduler')}
                        activeDragElement={activeDragElement}
                        setActiveDragElement={setActiveDragElement}
                        zIndex={getZIndex('scheduler')}
                    />
                )}
                ...
                {showAlerts && (
                    <AlertCenter
                        ...
                        position={elementPositions.alerts}
                        onMouseDown={(e) => handleMouseDown(e, 'alerts')}
                        activeDragElement={activeDragElement}
                        setActiveDragElement={setActiveDragElement}
                        zIndex={getZIndex('alerts')}
                    />
                )}
                ...
                {showIntegrations && (
                    <IntegrationPanel
                        ...
                        position={elementPositions.integrations}
                        onMouseDown={(e) => handleMouseDown(e, 'integrations')}
                        activeDragElement={activeDragElement}
                        setActiveDragElement={setActiveDragElement}
                        zIndex={getZIndex('integrations')}
                    />
                )}
                ...
                {showMessaging && (
                    <MessagingSettings
                        ...
                        position={elementPositions.messaging}
                        onMouseDown={(e) => handleMouseDown(e, 'messaging')}
                        activeDragElement={activeDragElement}
                        setActiveDragElement={setActiveDragElement}
                ...
                {showPlugins && (
                    <PluginManager
                        ...
                        position={elementPositions.plugins}
                        onMouseDown={(e) => handleMouseDown(e, 'plugins')}
                        activeDragElement={activeDragElement}
                        setActiveDragElement={setActiveDragElement}
                        zIndex={getZIndex('plugins')}
                    />
                )}
                ...
                {showModelSettings && (
                    <ModelSettings
                        ...
                        position={elementPositions.modelSettings}
                        onMouseDown={(e) => handleMouseDown(e, 'modelSettings')}
                        activeDragElement={activeDragElement}
                        setActiveDragElement={setActiveDragElement}
                        zIndex={getZIndex('modelSettings')}
                    />
                )}
                ...
                {showMemoryViewer && (
                    <MemoryViewer
                        ...
                        position={elementPositions.memoryViewer}
                        onMouseDown={(e) => handleMouseDown(e, 'memoryViewer')}
                        activeDragElement={activeDragElement}
                        setActiveDragElement={setActiveDragElement}
                        zIndex={getZIndex('memoryViewer')}
                    />
                )}
